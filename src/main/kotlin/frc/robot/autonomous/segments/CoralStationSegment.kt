...
/**
 * A segment which starts at a side of the reef, Goes to one of the coral stations to intake from ground
 * and returns to One of the 3 sides close to it.
 *
 * @param coralStation - Which coral station to go to.
 * @param startingSide - The side from which a path to the coral
 * station will be followed. When the coral station is KL, this can only be
 * AB, KL or IJ. When the coral station is CD this can only be AB, CD or EF.
 * @param endingSide - The side to which the robot will return to from the coral
 * station. When the coral station is KL, this can only be
 * AB, KL or IJ. When the coral station is CD this can only be AB, CD or EF.
...
class CoralStationSegment(
	...
	override fun generateCommand(alliance: Alliance) = withName("Intake near coral station ${coralStation.name}") {
		SwerveSubsystem.followPathCommand(
			...
			PathPlannerPath.fromPathFile("${coralStation.name} coral station to ${endingSide.name}-far"),
			alliance == Alliance.Red,
